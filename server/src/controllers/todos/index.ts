...
export const updateTodo = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!body.title || !body.status || !id) {
        res.status(401).json({
            status: 401,
            errorMessage: 'VallidationError: _id or required body proprtties is not defined'
        })
    ...
    if (!updatedTodo) {
        res
            .status(501)
            .json({ status: 501, message: 'Edit todo failed. Not implemented'})
...
export const deleteTodo = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!id) {
        res.status(401).json({
            status: 401,
            errorMessage: 'ValidationError: _id is not defined'
        })
